...
class SubsequenceReplayElement(struct.PyTreeNode):
    ...
    is_terminal: bool
    last_state_in_trajectory: np.ndarray[bool]
...
class SubsequenceReplayBuffer(ReplayBuffer):
    ...
    def _construct_batch_sample(self, index, first_terminal_index, n, gamma):
        states_stack = self._observation_stack[
            mod_index_range(index - self._stack_size + 1, index + self._spr_window, self._max_capacity)
        ]
        ...
        trajectory_end_flags = self._is_terminal_stack[spr_obs_indices] | self._is_truncation_stack[spr_obs_indices]
        # Sets mask to True for terminal/truncating state if trajectory ends
        last_state_in_trajectory = np.concatenate(([True], (1 - trajectory_end_flags).cumprod()[:-1]))

        return SubsequenceReplayElement(
            states_stack, actions_stack, reward, next_state, is_terminal, last_state_in_trajectory
        )
