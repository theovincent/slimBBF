...
class JaxSubsequenceParallelEnvReplayBuffer(object):
    ...
    def __init__(
        ...
        logging.info("Creating a %s replay memory with the following parameters:", self.__class__.__name__)
        ...
        self._cumulative_discount_vector = np.array(
            [math.pow(self._gamma, n) for n in range(update_horizon + 1)], dtype=np.float32
        )
    ...
    def _add_transition(self, transition):
        ...
        self.add_count += 1
        self.invalid_range = invalid_range(self.cursor(), self._replay_length, self._stack_size, self._update_horizon)
    ...
    def _check_add_types(self, *args):
        ...
        for i, (arg_element, store_element) in enumerate(zip(args, self.get_add_args_signature())):
            ...
            if arg_shape != store_element_shape:
                raise ValueError("arg {} has shape {}, expected {}".format(i, arg_shape, store_element_shape))
    ...
    def ravel_indices(self, indices_t, indices_b):
        return np.ravel_multi_index((indices_t, indices_b), (self._replay_length, self._n_envs), mode="wrap")
    ...
    def sample_transition_batch(
        ...
        update_horizons = jnp.ones(batch_size * subseq_len, dtype=jnp.int32) * (update_horizon - 1)
...
class PrioritizedJaxSubsequenceParallelEnvReplayBuffer(JaxSubsequenceParallelEnvReplayBuffer):
    ...
    def get_add_args_signature(self):
        ...
        add_signature = parent_add_signature + [ReplayElement("priority", (), np.float32)]
    ...
    def set_priority(self, indices, priorities):
        ...
        assert indices.dtype == np.int32, "Indices must be integers, " "given: {}".format(indices.dtype)
    ...
    def get_priority(self, indices):
        ...
        assert indices.dtype == np.int32, "Indices must be int32s, " "given: {}".format(indices.dtype)
